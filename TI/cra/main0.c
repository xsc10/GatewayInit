...
struct route_node *routelist;
int findRoutelist(char *route_find)
{
   struct route_node *tem=routelist;
   while(tem!=NULL)
   {
     if(strncmp(tem->routeIP,route_find,4)==0)return 1;
      tem=tem->next;
   }
   return 0;
}

void updateRoutelist(char *dhcphead)
{
   char *dhcpOptions=dhcphead+236+4;
   int i,n;
   struct route_node *add;
   ...
   while(*dhcpOptions!=0xFF)
   {
	 ...
	 for(i=0;i<n;i++)
	 {
	    if(findRoutelist(dhcpOptions+i*4)==0)//judge whether it already exists
	    {
	       add=(struct route_node *)malloc(sizeof(struct route_node));
	       strncpy(add->routeIP,dhcpOptions+i*4,4);
	       add->next=routelist;
	       routelist=add->next;
